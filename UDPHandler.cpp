...
class UDPHandler
{
	...
	static int UDPSend(UDPSocket sockfd, void* data, int length, SocketAddress* where)
	{
		#if UDP_Printout
			printf("UDPHandler::UDPSend(%i, %p, %i, %p)\n", sockfd, data, length, where);
		#endif
		return sendto(sockfd, data, length, 0, (const sockaddr*)(where), sizeof(sockaddr));
	}
	...
	static int UDPRecvFromWho(UDPSocket sockfd, void* where, int length, SocketAddress* who)
	{
		#if UDP_Printout
			printf("UDPHandler::UDPRecv(%i, %p, %i)\n", sockfd, where, length);
		#endif
