...
void Limit1() {
	ForEveryUsingTalon(
		ConfigLimitMode(
			CANTalon::LimitMode::kLimitMode_SoftPositionLimits
		)
	);
}
void Limit2() {
	ForEveryUsingTalon(
		ConfigLimitMode(
			CANTalon::LimitMode::kLimitMode_SwitchInputsOnly
		)
	);
}
void Limit3() {
	ForEveryUsingTalon(
		ConfigLimitMode(
			CANTalon::LimitMode::kLimitMode_SrxDisableSwitchInputs
		)
	);
}
void OpenFHLimit() {
	ForEveryUsingTalon(
		ConfigFwdLimitSwitchNormallyOpen(true)
	);
}
void CloseFHLimit() {
	ForEveryUsingTalon(
...
void OpenRHLimit() {
	ForEveryUsingTalon(
		ConfigRevLimitSwitchNormallyOpen(true)
	);
}
void CloseRHLimit() {
	ForEveryUsingTalon(
...
void SetDT_PulseWidth() {
	ForEveryUsingTalon(
		SetFeedbackDevice(CANTalon::FeedbackDevice::PulseWidth)
	);
}
void SetQPosition() {
...
void SetBrakeCoastModeB() {
	ForEveryUsingTalon(
		ConfigNeutralMode(
			CANTalon::NeutralMode::kNeutralMode_Brake
		)
	);
}
...
void SetStatusFrameRates_G() {
	...
	ForEveryUsingTalon(
		SetStatusFrameRateMs(
			CANTalon::StatusFrameRate::StatusFrameRateGeneral,
			set);
	)
}
void SetStatusFrameRates_F() {
	...
	ForEveryUsingTalon(
		SetStatusFrameRateMs(
			CANTalon::StatusFrameRate::StatusFrameRateFeedback,
			set);)
}
...
void SetStatusFrameRates_A() {
	...
	ForEveryUsingTalon(
		SetStatusFrameRateMs(
		CANTalon::StatusFrameRate::StatusFrameRateAnalogTempVbat,
		set);)
}
...
void ChangeTalons() {
	while(true) {
		...
		CANTalon* t = PossiblyCreateTalon(
			&GraphableDevices, &AllTalons, ID);
...
void ConfigReverseHLimit() {
	...
	menu(2, FunctionNames,
	"Open/Close Reverse Hard Limit Switch",
	 FunctionPointers, false);
}
void SetEncoderDirection() {
	char* FunctionNames[] = { "Forward",
	                          "Reverse"};
	VoidFunction FunctionPointers[] = {SetDirectionEF, SetDirectionER};
	menu(2, FunctionNames,
	"Set Encoder Direction",
...
void SetOutputDirection() {
	char* FunctionNames[] = { "Forward",
	                          "Reverse"};
	VoidFunction FunctionPointers[] = {SetDirectionOF, SetDirectionOR};
	menu(2, FunctionNames, "Set Output Direction",
	 FunctionPointers,
	 false);
}
void GetDIO_Pin() {
	int ID = 0;
	while(!(0 <= (
		ID = static_cast<int>(query("DigitalInput pin"))) && ID <= 9));
	printf("ID == %i\n", ID);
	DigitalInput* t = PossiblyCreateDigitalInput(
		&GraphableDevices, &AllDigitalBoths, ID);
	printf("Value == %i\n", t->Get());
}
void SetDIO_Pin() {
	int ID = 0;
	while(!(0 <= (
		ID = static_cast<int>(query("DigitalOutput pin"))) && ID <= 9));
	printf("ID == %i\n", ID);
	int value = static_cast<int>(query("Value?"));
	DigitalOutput* t = PossiblyCreateDigitalOutput(
		&GraphableDevices, &AllDigitalBoths, ID);
	t->Set(value);
	printf("Value == %i\n", value);
}
void SetControlMode() {
	...
	VoidFunction FunctionPointers[] = {
		...
		SetControlModeSl,
		SetControlModeCu};
	menu(6, FunctionNames,
		"Select New Control Mode", FunctionPointers, false);
}
void SetStatusFrameRates() {
	...
	VoidFunction FunctionPointers[] = {
		SetStatusFrameRates_G,
		SetStatusFrameRates_F,
		SetStatusFrameRates_Q,
...
class Robot : public SampleRobot {
	...
	void OperatorControl() {
		...
		VoidFunction FunctionPointers[] = {
			...
			Get};
		while(true) {
			menu(5, FunctionNames,
				"Select Action", FunctionPointers, true);
...
void menu(int length, char* options[], char* title, VoidFunction* funcs,
	...
	do {
		...
		int BannerLength = strlen(title) + SpacePadding * 2 + 7 +
			TalonsCurrentlyUsing.size();
