 ...
 */
Sequence0C::Sequence0C() : frc::CommandGroup("Sequence0C") {
  AddSequential(new deadeye::EnableCamera(deadeye::EnableCamera::Mode::gear));

  bool is_practice = RobotMap::IsPracticeRobot();

  std::string msg = is_practice ? "RED alliance right gear on PRACTICE (08)"
                                : "RED alliance right gear on COMP (08)";
  ...
  dc.max_speed = 200;
  dc.segments.clear();
  dc.segments.emplace_back(-60, 20 * kTicksPerInch);
  AddSequential(new drive::Drive(dc));

  // pivot back towards boiler
  AddSequential(new drive::GyroAzimuth(-35));
