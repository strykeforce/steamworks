	...
	class SocketHandler
	{
		...
		static int FilloutSocketAddress(SocketAddress* serveraddr, char* hostname, short portno);

		static int TCPConnectToServer(Socket sockfd, SocketAddress* where);

		static Socket TCPWaitForClient(Socket sockfd, SocketAddress* client_address=NULL);

		static int TCPSend(Socket sockfd, void* data, int length);

		static int UDPSend(Socket sockfd, void* data, int length, SocketAddress* where);

		static int Recv(Socket sockfd, void* where, int length);

		static int SetTimeout(Socket sockfd, int microseconds, int seconds = 0);

		static int RecvFromWho(Socket sockfd, void* where, int length, SocketAddress* who);
