...
Socket SocketHandler::CreateTCPSocketSERVER(unsigned short portno)
{
	...
	{
		printf("SocketHandler::CreateTCPSocketSERVER:\tERROR on binding to port %i\n", portno);
	}
	listen(sockfd, 5);
	int optval = 1;
	int returned = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (const void *)&optval, sizeof(int));
	return sockfd;
}
int SocketHandler::FilloutSocketAddress(SocketAddress* serveraddr, char* hostname, short portno)
{
	struct hostent *server = gethostbyname(hostname);
	if(server == NULL)
	{
		printf("SocketHandler::FilloutSocketAddress: Couldn't get host address of \"%s\"\n", hostname);
		switch(h_errno)
		{
			case HOST_NOT_FOUND:	{printf("\tHOST_NOT_FOUND (The specified host is unknown.)\n"); break;}
			case NO_ADDRESS:	{printf("\tNO_ADDRESS or NO_DATA (The requested name is valid but does not have an IP address.)\n"); break;}
			case NO_RECOVERY:	{printf("\tNO_RECOVERY (A nonrecoverable name server error occurred.)\n"); break;}
			case TRY_AGAIN:		{printf("\tTRY_AGAIN (A temporary error occurred on an authoritative name server.  Try again later.)\n"); break;}
		}
		return h_errno;
	}
...
int SocketHandler::UDPSend(Socket sockfd, void* data, int length, SocketAddress* where)
{
	int returned = sendto(sockfd, data, length, 0, (const sockaddr*)(where), sizeof(sockaddr));
	return returned;
}
...
int SocketHandler::RecvFromWho(Socket sockfd, void* where, int length, SocketAddress* who)
{
