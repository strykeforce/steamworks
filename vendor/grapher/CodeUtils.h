	...
	#define ssleep(__SECONDS__)\
		msleep(__SECONDS__ * 1000);

	#define msleep(__MILLISECONDS__)\
		usleep(__MILLISECONDS__ * 1000);
	...
	double BasicPositionControl(double Error, double Start_Slope, double Close_Enough, double MinSpeed, double MaxSpeed, int* counter = NULL);
	byte* ReadWholeFile(char* pathname);
	void RemoveComments(char* orginal);
	template <typename T> T Limit (T a, T b, T c);
#endif
